...
Spaz.Dock.init = function() {
	...
	//    console.info("Want to load image " + imageURL);
...
Spaz.Dock.sync = function()
{
   ...
   if (reloadID != null)
   {
      // console.info("Stopping dock refresh thread");
   ...
   if (Spaz.Prefs.getDockDisplayUnreadBadge())
   {
		...
		//       console.info("Starting dock refresh thread with refresh rate of " + refresh + " ms");
...
Spaz.Dock.refresh = function(unreadCount)
{
   ...
   if (diff < Spaz.Dock.mincachetime) {
           // console.info('timelimit not passed (it is now '+now+')');

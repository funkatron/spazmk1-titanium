...
Spaz.Data.getAPIURL = function(key) {
	...
	if (urls[key]) {
		// console.info("URL:"+base_url + urls[key]);
...
Spaz.Data.verifyPassword = function() {
	...
	var xhr = $.ajax({
		...
		success:function(data){
			...
			// if (json.authorized) {
				// Spaz.verified = true;
				console.info('verified; setting current user');
	...
	if (Spaz.Data.$ajaxQueueFinished >= section.urls.length) {
		...
		if (Spaz.Data.$ajaxQueueStorage.length > 0) {
			...
			for (var i in Spaz.Data.$ajaxQueueStorage) {
				// console.info('URL:'+thisurl);
				// console.info('section URLs:'+section.urls);
				/*
...
Spaz.Data.getDataForUrl = function(url, section) {
	...
	var xhr = $.ajax({
		...
		complete:function(xhr, msg){
			console.info('Getting data for url:'+url);
...
Spaz.Data.getRateLimitInfo = function(callback, cbdata) {
	...
	if (!user || !pass) {
		console.info('Dropping out of getRateLimitInfo because user or pass is not set');
		return false;
	}

...
Spaz.Cache.buildScreenNameCache = function() {
	...
	// 	console.info("numEntries is "+ numEntries + " > " + Spaz.Prefs.get('screennames-cache-max') + "; removing last "+diff+" entries");
	// 
	// 	Spaz.Cache.screenNames.splice(0, diff);
	// }
	// 
	// 
	// console.info('Spaz.Cache.screenNames = '+Spaz.Cache.screenNames.toString() + ' ['+Spaz.Cache.getScreenNamesCount()+']');
};
...
Spaz.Cache.delScreenName = function(name) {
	if (Spaz.Cache.screenNames.indexOf(name)) {
		console.info('Spaz.Cache.delScreenName not yet implemented');
	}
...
Spaz.Cache.getScreenNames = function() {
	...
	$('[user-screen_name]', '#'+Spaz.Section.friends.timeline).each(function() {
		...
		if (screen_names.indexOf(name) == -1) {
			...
			// console.info('Added "'+name+'". Number of screen names is '+screen_names.length);
		}
	});

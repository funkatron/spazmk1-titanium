...
Spaz.Keyboard.move = function(dir, selector) {
	...
	// console.info('selected:'+jqsel.length);
	// console.info('moving:'+movefunc+'/'+wrapselc+"\n"+'selected:'+jqsel.length);
	
	// if none selected, or there is no 'next', select first
	if (selector == ":first" || selector == ":last") {
		// console.info('first in timeline');
		...
		// console.info('we are at the beginning or end');
		if (Spaz.Prefs.get('timeline-keyboardnavwrap')) {
			Spaz.Keyboard.moveSelect($('#'+timelineid+' div.timeline-entry:visible'+selector+':'+wrapselc), section)
			jqsel = $('#'+timelineid+' div.timeline-entry.ui-selected'+selector);				
		} else {
			console.info('NOT WRAPPING');
		}
	} else {
		// console.info('something is now selected');
...
Spaz.Keyboard.moveSelect = function(jqelement, section) {	
	...
	if (jqelement.length > 0) {
		console.info("toggle ui-selected and scrollto");
		...
		*/
		// console.info('top---')
		// console.info(jqelement.position().top)
		// console.info($('#'+section.wrapper).position().top)
		...
		console.info("Problem - jqelement passed length = 0 - can't scroll to anything");
	}

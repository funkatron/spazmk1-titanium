...
Spaz.Prefs.changeMethods = {
	...
	'network-airhandlehttpauth': {
		...
		onChange: function(value) {
			...
			Spaz.dump('network-airhandlehttpauth not functional');
			// window.htmlLoader.authenticate = value;
		},
		check: function() {
...
Spaz.Prefs.init = function() {
	...
	*/
	...
	console.info('set sound file locations');
	Spaz.Prefs.initUI();
	console.info('initUI from prefs');
}
...
Spaz.Prefs.loadPrefs = function() {
	...
	prefsFile = Titanium.Filesystem.getFile(appStore.nativePath(), 'preferences.json');

	console.dir(prefsFile);

	if (prefsFile.exists()) {
		console.info("File exists, reading…");
		var prefsJSON = prefsFile.read().toString();
		Spaz.dump(prefsJSON);
		console.debug(prefsJSON);
		var loadedpreferences = JSON.parse(prefsJSON);
		console.dir(loadedpreferences);
		...
		console.info('Writing prefs file');
		prefsFile.write(JSON.stringify(Spaz.Prefs.defaultPreferences));
		console.info('Wrote prefs file');
		Spaz.Prefs.preferences = clone(Spaz.Prefs.defaultPreferences);
	}

	Spaz.Prefs.loadUsername();
	console.info('Loaded username');
	Spaz.Prefs.loadPassword();
	console.info('Loaded password');
...
Spaz.Prefs.setSoundFileLocations = function() {
	Spaz.dump("Setting sound file locations TODO!!");
		// 
		// var soundFileUpdate = Titanium.Filesystem.getApplicationDataDirectory();
		// soundFileUpdate = soundFileUpdate.resolvePath('usersounds/Csnd.mp3');
		// 
		// var soundFileStartup = Titanium.Filesystem.getApplicationDataDirectory();
		// soundFileStartup = soundFileStartup.resolvePath('usersounds/On.mp3');
		// 
		// var soundFileShutdown = Titanium.Filesystem.getApplicationDataDirectory();
		// soundFileShutdown = soundFileShutdown.resolvePath('usersounds/Off.mp3');
		// 
		// var soundFileNew = Titanium.Filesystem.getApplicationDataDirectory();
		// soundFileNew = soundFileNew.resolvePath('usersounds/New.mp3');
		// 
		// var soundFileWilhelm = Titanium.Filesystem.getApplicationDataDirectory();
...
Spaz.Prefs.savePrefs = function() {
	...
	var prefsFile = Titanium.Filesystem.getApplicationDataDirectory();
	prefsFile = prefsFile.resolvePath("preferences.json");
	prefsFile.write(JSON.stringify(Spaz.Prefs.preferences));
};
...
Spaz.Prefs.saveUsername = function() {
	Spaz.Prefs.set('twitter_username_1', Spaz.Prefs.user);
	// if (var username = Spaz.Prefs.set('twitter_username_1')) {
	//	return username
	// } else {
	//	return false;
	// };
...
Spaz.Prefs.loadUsername = function() {
	...
	if (username = Spaz.Prefs.get('twitter_username_1')) {
		return username;
	} else {
		return false;
	};
...
Spaz.Prefs.savePassword = function() {
	...
	if (Spaz.Prefs.pass) {
		Spaz.Prefs.set('twitter_password_1', Spaz.Prefs.set);
...
Spaz.Prefs.loadPassword = function() {
	...
	if (password = Spaz.Prefs.get('twitter_password_1')) {
		return password;
	} else {

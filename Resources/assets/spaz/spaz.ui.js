...
Spaz.UI.onSoundPlaybackComplete = function(event) {
    console.info("The sound has finished playing.");
}
...
Spaz.UI.doWilhelm = function() {
	...
	if (Spaz.Prefs.get('window-dropshadow')) {
	    console.info('Applying Flash Filter Dropshadow');
...
Spaz.UI.prepRetweet = function(entryid) {
	...
	// console.info(timelineid);
	// console.info('#'+timelineid+'-'+entryid);
	var entry = $('#'+timelineid+'-'+entryid);
	// console.info(entry.html());
...
Spaz.UI.setView = function(type ) {
	...
	console.info('View type is '+type);
...
Spaz.UI.showLinkContextMenu = function(jq, url) {
    ...
    // console.info('hiding tooltip');
    $('#tooltip').hide();

    // show the link context menu
    // console.info('opening context menu');
    ...
    // console.info('outerHTML:'+el.outerHTML);
    var urlarray = /http:\/\/([^'"]+)/i.exec(url);
    if (urlarray && urlarray.length > 0) {
        var elurl = urlarray[0];

        // console.info('url from element:'+elurl);
        $('#linkContextMenu-copyLink').one('click', {
            url: elurl
        },
        function(event) {
            Spaz.Sys.setClipboardText(event.data.url);
            // console.info('Current Clipboard:'+Spaz.Sys.getClipboardText());
        });
        // console.info('Set one-time click event on #menu-copyLink');
        $(document).one('click',
        function() {
            $('#linkContextMenu').hide();
        });
        // console.info('set one-time link context menu close event for click on document');
    } else {
        // console.info('no http link found');
        }
};
...
Spaz.UI.showUserContextMenu = function(jq, screen_name) {
    ...
    // show the link context menu
    // console.info('opening context menu for user '+screen_name);
    ...
    // console.info('Set one-time click event on #userContextMenu');
    $(document).one('click',
    function() {
...
Spaz.UI.addItemToTimeline = function(entry, section, mark_as_read, prepend) {
	...
	console.info('TIMELINE #' + timelineid + '-' + entry.id);
	
	

    console.info(JSON.stringify(entry));
    if ($('#' + timelineid + '-' + entry.id).length < 1) {
		console.info('adding #' + timelineid + '-' + entry.id);
		entry.isDM = false;
		entry.isSent = false;
		...
		*/
		...
		if (entry.in_reply_to_user_id && !entry.in_reply_to_screen_name) {
			console.info('in_reply_to_user_id exists, but in_reply_to_screen_name not set');
		...
		*/
		...
		// console.info(entryHTML);
		
		// console.info('make DOMElement from string: '+entryHTML);
		// var entryElement = document.createElement(entryHTML);
		
		// Make the jQuery object and bind events
		// console.info('make jQ entry and bind events to: '+entryElement);
		// console.info('make jQ entry and bind events to: '+entryElement);
		...
		if (typeof mark_as_read == 'undefined')
		{
			...
			function(read)
			{
				if (read)
				{
					// console.info("Entry " + entry.id + " is read");
					jqentry.addClass('read');
					jqentry.removeClass('new');
					$().trigger('UNREAD_COUNT_CHANGED');
				}
...
Spaz.UI.selectEntry = function(el) {
    ...
    $(el).addClass('ui-selected').addClass('read').each(function() {
        if (entryId = Spaz.UI.getStatusIdFromElement(this)) {
            console.info("Want to mark as read " + entryId);
...
Spaz.UI.getStatusIdFromElement = function(el) {
    ...
    //
    if (entryId == null) {
        console.info("Cannot obtain entry id for entry with DOM id " + this.id);
        return false
    } else {
        return entryId
    }
...
Spaz.UI.cleanupTimeline = function(timelineid, suppressNotify, suppressScroll, skip_sort) {
	...
	*/
	...
	console.info("oldFirst BEFORE top: " + oldFirst.offset().top + "\nscrollTop: " + $timeline.parent().scrollTop());
    ...
    Spaz.Timers.add(function() {
		...
		function removeExtraItems(type) {
	        ...
	        if (numEntries > Spaz.Prefs.get(prefkey)) {
	            ...
	            console.info("numEntries "+type+"is " + numEntries + " > " + Spaz.Prefs.get(prefkey) + "; removing last " + diff + " entries");
            ...
            if ($("#" + timelineid + ' .timeline-entry.new:not(.read)').length > 0) {
				...
				*/
                if (Spaz.Prefs.get('timeline-scrollonupdate') && $timeline.parent().scrollTop() > 0) {
                    try {
						console.info('scrolliong thee timeline '+timelineid);
        ...
        cleanupTweets.find("div.status-text").each(function(i) {
            ...
            for (var i in domains)
            {
                ...
                while (matchArray = urlRE.exec(txt)) {
                    // console.info("Getting content of URL " + matchArray[1]);
					...
					// Perform load
                    stream.load(url_req);
                    // console.info("Decoding " + domain + " URL " + url);
                    console.info("Decoding " + domain + " URL " + url);
                }
            ...
            function onHTTPResponseStatus(event) {
                // console.info('onHTTPResponseStatus');
                if (event.status == 200) {
		...
		function fadeInStragglers() {
			$("img.user-image").each( function() {
				if ($(this).css('opacity') < 1) {
					// console.info('fadingIn: '+this.outerHTML);
					$(this).fadeTo('500', '1.0');
				}
			});	
		};

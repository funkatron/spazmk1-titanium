...
Spaz.createUserDirs = function() {
	...
	var userdirs = ['userthemes', 'userplugins', 'usersmileys', Spaz.Const.userdirSound];
	
	function createUserDir(dirname) {
		var appStore = Titanium.Filesystem.getApplicationDataDirectory();
		var new_directory = Titanium.Filesystem.getFile(appStore.nativePath(), dirname);
		if (!new_directory.exists()) {
			new_directory.createDirectory();
		}
		console.info(new_directory.nativePath());
	}

	
	for (var i=0; i < userdirs.length; i++) {
		createUserDir(userdirs[i]);
	};
	...
	// 
	// console.info(userThemesDir.nativePath);
	// console.info(userPluginsDir.nativePath);
	// console.info(userSmileysDir.nativePath);
	// console.info(userSoundsDir.nativePath);
};
...
Spaz.initialize = function() {
	 ...
	 * Load prefs
	 **************************/
	console.info('init prefs');
	Spaz.Prefs.init();

	console.info('init Sections');
	Spaz.Section.init();

		...
		onAjaxComplete: function(data, msg) {
			...
			if (data && data.results) {
				...
				function summizeToTweet(result) {
					...
					// console.info(JSON.stringify(tweet));
					return tweet
				}
	...
	Spaz.Section.friendslist = {
				...
				for (var i = 0; i < this.urls.length; i++) {
					...
					var xhr = $.ajax({
						...
						complete:function(xhr, msg){
							// console.info(JSON.stringify(this.url));
							// console.info('Getting data for url:'+this.url);
		...
		onAjaxComplete: function(url, xhr, msg) {
			...
			if (xhr.readyState < 3) { // XHR is not yet ready. don't try to access response headers
				...
				console.info('this.$ajaxQueueFinished:'+this.$ajaxQueueFinished);
				console.info('this.urls.length:'+this.urls.length);
				console.info('data.length:'+data.length);
				console.info('this.$ajaxQueueErrors.length:'+this.$ajaxQueueErrors.length);
			}

			console.info('Adding '+data.length+' entries from '+url);
			...
			if (data.length > 0) {
				...
				for (var i in data) {
					...
					console.info(this.friends_ids.length + ":" + this.followers_ids.length);
					
					if ( this.friends_ids.indexOf(data[i].id) > -1 ) {
						console.info(data[i].id + " is your friend");
						data[i].is_following = true;
					}
					
					if ( this.followers_ids.indexOf(data[i].id) > -1 ) {
						console.info(data[i].id + " is your follower");
						data[i].is_follower = true;
					}
					
					if (data[i].is_following && data[i].is_follower) {
						console.info(data[i].id + " is your friend and follower");
